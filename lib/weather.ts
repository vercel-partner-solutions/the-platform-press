...
const WEATHER_CODE_MAP: Array<{ codes: number[]; condition: WeatherCondition }> = [
    { codes: [0], condition: "Sunny" },
    { codes: [1, 2, 3, 45, 46, 47, 48], condition: "Cloudy" },
    { codes: [51, 52, 53, 54, 55, 56, 57, 61, 62, 63, 64, 65, 66, 67, 80, 81, 82, 95, 96, 99], condition: "Rainy" },
...
async function getCoordinatesFromCity(city: string): Promise<{ latitude: number; longitude: number } | null> {
...
async function getWeatherFromCoordinates(latitude: number, longitude: number, locale: string): Promise<WeatherData> {
    ...
    return {
        ...
        unit: getTemperatureSymbol(temperatureUnit),
    };
}
